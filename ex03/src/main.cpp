...
{
	printHeader("TEST 13: Intern::makeForm - Valid Forms");
	try
	{
		Intern intern;
		...
		AForm *f2 = intern.makeForm("RobotomyRequestForm", "Bender");
		boss.signForm(*f2);
		boss.executeForm(*f2);
		delete f2;
...
{
	printHeader("TEST 14: Intern::makeForm - Unknown Form");
	try
	{
		Intern intern;
		AForm *f = intern.makeForm("UnknownForm", "target");
		delete f;
	}
	catch (const std::exception& e)
	{
